...
function NewProductCard(
  ...
  const addToCartButtonClassNames = (variant: string | undefined) =>
    `!bg-[#2C376D] h-10 w-full font-poppins not-italic font-semibold text-white btn btn-${
  ...
  return (
    <div
      class={`bg-white rounded-[20px] border border-[#E5E7EB] card card-compact opacity-100 bg-opacity-100 group w-full max-w-[240px] lg:max-w-none p-[15px] ${
    ...
    >
      ...
      >
        ...
        {freeShipping && (
          <FreeShipping
            classNameContainer={`md:left-3 left-1 ${
              listPrice2 !== price2 ? `top-11` : `top-0`
            } ${classFrreShipping}`}
            classNameIcon="mr-2"
          />
        )}
                  ...
                  <div class="flex flex-col items-start mt-[10px] h-auto">
                    ...
                    >
                      {(listPrice && price) && listPrice > price && (
                        <p
                          class={`line-through text-base-300 text-xs lg:text-xs  ${
                            l?.basics?.oldPriceSize === "Normal"
                              ? "lg:text-xl"
                              : ""
                          }`}
