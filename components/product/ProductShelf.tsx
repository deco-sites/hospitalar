...
function ProductShelf({ products, title, layout, cardLayout, showPaginationArrows, IdCollection, slideTwoRow = false }: Props) {
    const id = useId();
    const device = useDevice();
    if (!products || products.length === 0) {
        return null;
    }
    function redesignArray(products: [
    ]): [
        Product[]
    ] {
        let arrayProducts = [];
    ...
    return (<div class={clx(`${slideTwoRow
      ...
      <CustomHeader title={title || ""} description="" fontSize={layout?.headerfontSize || "Large"} alignment={layout?.headerAlignment || "center"} color={layout?.color || "primary"}/>
      </div>
            ...
            ? (<Slider class={clx(`container carousel carousel-start flex max-lg:overflow-hidden`)}>
              ...
              {device === 'mobile' && products?.map((product, index) => {
                    return (<Slider.Item index={index} class={clx(`carousel-item w-[335px]`)}>
                      <NewProductCard product={product} itemListName={title} layout={cardLayout} IdCollection={IdCollection ?? "156"} tagWarningWidth="40%"/>
                    </Slider.Item>);
                })}
              {slideTwoRow && device === 'desktop' && redesignArray(products).map((produtosInternos, index) => {
                    ...
                    {produtosInternos?.map((product) => (<NewProductCard product={product} itemListName={title} layout={cardLayout} IdCollection={IdCollection ?? "156"} tagWarningWidth="40%"/>))}
                  </Slider.Item>);
                })}          
            </Slider>)
            : (<Slider class={clx(`container carousel carousel-start gap-6 col-span-full row-span-full py-2 mb-8 lg:pb-[20px] lg:mb-0'`)}>
        ...
        <>
          <div class={clx(`flex items-center justify-center z-10 col-start-1 row-start-2  ${CONDITIONAL_RESPONSIVE_PARAMS[showPaginationArrows ? showPaginationArrows : "Always"]}
            ...
            minHeight: "28px",
              ...
              <Icon size={32} id="LeftArrowFigma"/>
            </Slider.PrevButton>
          </div>
          <div class={clx(`flex items-center justify-center z-10 col-start-3 row-start-2 ${CONDITIONAL_RESPONSIVE_PARAMS[showPaginationArrows ? showPaginationArrows : "Always"]}
