...
function VariantSelector(
  { isLine, product, onChangeVariant }: {
    isLine: boolean;
    product: Product;
    onChangeVariant: (sku: string, newName: string) => void;
  },
) {
  ...
  return (
    <div
      className={`flex flex-col ${
        !isLine ? "items-center justify-center" : ""
      } mt-3 gap-3 w-full lg:items-center lg:justify-center`}
    >
      <span className="font-poppins font-medium text-xs text-[#2D386E]">
        Selecione a opção
      </span>
      <select
        onChange={(e) => {
      ...
      >
        <option
          value={`${name}-${sku}`}
          className="font-poppins font-normal text-sm text-[#999BA2]"
        >
          {!isLine ? "Selecione" : "Selecione aqui"}
        </option>
        {possibilities.map((variant, index) => {
          return (
            <option
              className="font-poppins font-normal text-sm text-[#999BA2]"
              value={`${variant.name}-${variant.variantProductID}`}
              key={index}
            >
              {variant.name}
            </option>
...
function CardResult({ quantity, totalListPrice, totalPrice, items }: {
  ...
  const formatMoney = new Intl.NumberFormat("pt-BR", {
    style: "currency",
    currency: "BRL",
  }).format(totalListPrice - totalPrice);
  const formatMoneySlash = new Intl.NumberFormat("pt-BR", {
    style: "currency",
    currency: "BRL",
  }).format(totalListPrice);
  const formatMoneyProm = new Intl.NumberFormat("pt-BR", {
    style: "currency",
    currency: "BRL",
  }).format(totalPrice);
  ...
  return (
    <div className="bg-[#F8F8F9] m-full rounded-2xl px-4 py-5 mt-10 relative lg:mt-0 lg:flex lg:justify-center lg:items-center">
      ...
      <div className="flex flex-col gap-3 items-center justify-center">
        <strong className="text-[#0D4F81] text-center font-poppins font-bold text-xl uppercase">
          Leve {quantity} Produtos
        </strong>
        <div className="flex flex-col items-center justify-center gap-4">
          {(totalListPrice - totalPrice !== 0) && (
            <div className="flex items-center justify-center rounded border border-[#2D386E] w-52 py-1 px-2">
              <span className="text-[#0D4F81] text-center font-poppins font-semibold text-xs">
          ...
          <div className="flex flex-col">
            {(totalListPrice !== totalPrice) && (
              <span className="text-[#1B1C17] text-center font-poppins text-xs font-normal line-through">
                {formatMoneySlash}
              </span>
            )}
            <strong className="text-[#0D4F81] text-center font-poppins text-sm font-bold">
              {formatMoneyProm} à vista
            </strong>
          </div>
        </div>
        <Button
...
function ProductTogether({
  ...
  return (
    <div
      className={`rounded-2xl border relative border-[#D7D7DA] ${
        isLine ? "py-[30px] px-[15px] mb-" : "pb-5 px-3"
      } ${isLine && "flex flex-col justify-between"} lg:py-[20px] px-[14px]`}
    >
      <div
        className={`flex ${
          isLine ? "flex-row" : "flex-col"
        } items-center justify-center gap-5 lg:flex-col`}
      >
        <a
          href={url}
          className={`${
            isLine ? "w-[120px] h-auto" : "w-full h-auto"
          } lg:w-full xl:max-w-52`}
        >
          <img
        ...
        <div
          className={`${
            isLine ? "w-[calc(100%-120px)] lg:w-full" : "w-full"
          } gap-3 flex flex-col`}
        >
          <div className="overflow-hidden w-full truncate lg:overflow-visible lg:whitespace-normal">
            <span className="text-[#4A4B51] lg:block lg:w-full text-center font-poppins text-xs font-normal">
              {isVariantOf?.name || name}
            </span>
          </div>
          <div
            className={`w-full h-auto gap-[2px] flex flex-col justify-center ${
              isLine ? "items-start" : "items-center"
            } lg:items-center`}
          >
            <p
              className={`text-[#0D4F81] ${
                isLine ? "text-left" : "text-center"
              } font-poppins text-sm font-medium lg:text-center`}
            >
              <strong>{formatPrice(price)!.replace("\xa0", " ")}</strong>{" "}
              à vista <br />{" "}
              <>
                {installment && installment.billingDuration &&
                  installment.billingIncrement &&
                  installment.billingDuration > 1 && (
                  <>
                    ou {formatPrice(
            ...
            {installments && installment && installment.billingDuration &&
              installment.billingDuration > 1 && (
              <p className="text-[#1F2937] text-center font-poppins text-xs font-normal">
                em até {installments} sem juros
              </p>
            )}
          </div>
        </div>
...
function ShopTogetherContainer({ buyTogether }: IShopTogetherContainerProps) {
  ...
  useEffect(() => {
    ...
    buyTogetherReduce.forEach((product, _index) => {
  ...
  const handleToggle = (product: Product) => {
    setTogetherProducts((prevTogetherProducts) => {
      const productIndex = prevTogetherProducts.findIndex((p) =>
        p.mainSkuId === product.sku
      );

      const updatedProducts = [...prevTogetherProducts];
      if (productIndex > -1) {
      ...
      const newTotalListPrice = updatedProducts.reduce(
        (acc, p) => acc + (p.listPrice ?? 0),
        0,
      );
  ...
  return (
    <div
      className={`w-full h-auto flex flex-col lg:grid ${
    ...
    >
      <div
        className={`${
          buyTogetherLimited.length === 2
            ? "grid grid-cols-2 gap-3 w-full lg:!w-auto lg:gap-[42px] lg:grid-cols-[360px_360px]"
            : `flex flex-col gap-10 lg:grid lg:grid-cols-${buyTogetherLimited.length} lg:gap-5 xl:gap-[42px] w-full`
        } h-auto relative`}
      >
        {buyTogetherLimited.map((productTogether, index) => {
          return (
            <ProductTogether
              ...
              checked={togetherProducts.some((p) =>
                p.mainSkuId === productTogether.sku
              )}
            />
          );
        })}
