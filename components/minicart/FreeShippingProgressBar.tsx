    ...
    <div class="flex flex-col w-full gap-3 mb-4">
        ...
        {total < 49900
          ? (
            <span class="text-sm font-normal">
              {left_text ? left_text : "Faltam"}{" "}
              {formatPrice((49900 - total) / 100, currency, locale)}
              {right_text ? right_text : " para ganhar frete grátis*"}
            </span>
          )
      ...
      <div class="flex flex-row items-start justify-start h-auto w-full mt-[6px] gap-2">
        <span class="text-sm font-normal text-primary">
          {rule_text ? rule_text : "*Sul e Sudeste"}
        </span>
        <a
          class="text-sm font-normal text-primary underline decoration-1"
          href={redirect_link ? redirect_link : "#"}
        >
          {redirect_text ? redirect_text : "regulamento"}
        </a>
      </div>
    </div>
  );
