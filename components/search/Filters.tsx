...
function ValueItem({ url, selected, label, quantity }: FilterToggleValue) {
  return (
    <a href={`${url}&page=1`} rel="nofollow" class="flex items-center gap-2">
      ...
      <span class="font-poppins not-italic font-normal text-sm leading-5 text-[#4A4B51]">{label}</span>
      {quantity > 0 && <span class="font-poppins not-italic font-normal text-sm leading-5 text-[#8E8E9F]">({quantity})</span>}
    </a>
  );
}
...
function Filters({ filters }: Props) {
  ...
  return (
    <ul class="flex flex-col gap-2 text-primary">
      <li class="mb-[24px]">
        <p class="font-poppins not-italic font-semibold text-base leading-6 text-[#2C376D]">Filtrar por:</p>
      ...
      {_filters.map((filter) => (
        <li class="flex flex-col gap-4">
          <details class="collapse collapse-plus" open>
            <summary style={{ minHeight: "auto !important" }} class="bg-[#EEF1F5] rounded-[60px] !h-[45px] min-h-auto collapse-title mb-[16px]">
